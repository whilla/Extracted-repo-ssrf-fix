...
FAILURE TO FOLLOW THESE INSTRUCTIONS WILL RESULT IN REJECTION.`;
  ...
  async execute(context: AgentExecutionContext): Promise<AgentOutput> {
    ...
    try {
      let currentPrompt = await this.buildPrompt(context);
      let loopCount = 0;
      const maxToolLoops = 3;
      let lastContent = '';

      while (loopCount < maxToolLoops) {
        const finalPrompt = this.applyDeepReasoningGuidance(currentPrompt);
        
        await this.recordDecision({
          timestamp: new Date().toISOString(),
          summary: `Executing with provider ${context.provider.id} (Loop ${loopCount})`,
          selectedProvider: context.provider.id,
          outcome: 'started',
        });

        await this.updateHeartbeat('executing', `Executing with ${context.provider.id}`);
        const execution = await this.executeProvider(finalPrompt, context.provider);
        
        const rawContent = execution.content;
        const toolCalls = this.extractToolCalls(rawContent);

        if (toolCalls.length === 0) {
          lastContent = this.processOutput(rawContent, context);
          break;
        }

        // Execute tools and build a results string
        let toolResults = '';
        for (const call of toolCalls) {
          const result = await toolExecutorService.execute(call.name, call.args);
          toolResults += `\nTool ${call.name} Result: ${result.success ? result.content : 'Error: ' + result.error}\n`;
        }

        // Feed results back into the prompt for the next iteration
        currentPrompt += `\n\nAI output was: ${rawContent}\n${toolResults}\n\nPlease refine your response based on these tool results.`;
        loopCount++;
      }
      ...
      await this.updateHeartbeat('healthy', `Completed with ${context.provider.id}`);
      await this.recordDecision({
        timestamp: new Date().toISOString(),
        summary: `Completed successfully using ${context.provider.id}`,
        selectedProvider: context.provider.id,
        retries: 0,
        outcome: 'success',
      });
      ...
      return {
        ...
        content: lastContent,
        success: true,
        reasoning: `Generated by ${this.config.name} (v${this.state.evolutionVersion}) in ${duration}ms with ${loopCount} tool loops`,
        metadata: {
          duration,
          promptLength: currentPrompt.length,
          outputLength: lastContent.length,
